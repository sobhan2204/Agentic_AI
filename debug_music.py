...
print("=== DEBUGGING SPOTIFY API ===")

# Check if authentication is working
try:
    headers = auth.get_auth_headers()
    print("✓ Authentication successful")
    print(f"Access Token (first 20 chars): {auth.access_token[:20]}...")
except Exception as e:
    print(f"✗ Authentication failed: {e}")

# Test with a very simple API call first
print("\n=== TESTING BASIC API CALL ===")
test_search = search_tracks("test", limit=1)
print(f"Basic search works: {test_search['success']}")

if test_search['success'] and test_search['tracks']:
    test_track_id = test_search['tracks'][0]['id']
    print(f"Test track ID: {test_track_id}")
    print(f"Test track name: {test_search['tracks'][0]['name']}")
    
    # Try to get basic track info first
    print("\n=== TESTING TRACK DETAILS ===")
    track_details = get_track_details(test_track_id)
    print(f"Track details work: {track_details['success']}")
    
    # Now try audio features with better error handling
    print("\n=== TESTING AUDIO FEATURES WITH DEBUG INFO ===")
    try:
        headers = auth.get_auth_headers()
        response = requests.get(f'https://api.spotify.com/v1/audio-features/{test_track_id}', headers=headers)
        ...
        if response.status_code == 200:
            print("✓ Audio features request successful!")
        elif response.status_code == 403:
            print("✗ 403 Forbidden - This might be a scope/permission issue")
            print("  Audio features might require user authentication, not just client credentials")
        else:
...
if result['success'] and result['tracks']:
    real_track_id = result['tracks'][0]['id']
    result3 = get_audio_features(real_track_id)
else:
    result3 = {'success': False, 'error': 'No tracks found to get audio features'}
