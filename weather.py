...
async def _geocode(place: str) -> dict:
    ...
    display_name = ", ".join(
        [x for x in [city, admin1, country] if x]
    )
...
async def _fetch_weather(lat: float, lon: float) -> dict:
    ...
    params = {
        ...
        # Also fetch humidity for richer output
...
async def _fetch_aqi(lat: float, lon: float) -> dict:
    """Internal: fetch CURRENT air quality from Open-Meteo."""

    params = {
        "latitude": lat,
        "longitude": lon,

        # CURRENT AQI VALUES ONLY
        "current": (
            "pm2_5,"
            "pm10,"
            "carbon_monoxide,"
...
async def get_weather(city: str) -> str:
    ...
    try:
        ...
        return (
            f"Request timed out while fetching weather "
            f"for '{city}'. Please try again."
        )

    except httpx.HTTPStatusError as e:
        return (
            f"Weather API error for '{city}': "
...
async def get_air_quality(city: str) -> str:
    """
    Get the CURRENT air quality index and pollutant levels
    for any city or place.

    Use this tool when the user asks about air quality,
    pollution, AQI, PM2.5, smog, or whether the air is safe
    to breathe in a location.
    ...
    Returns:
        A human-readable air quality report with PM2.5,
        PM10, ozone, nitrogen dioxide, and carbon monoxide levels.
    """
    ...
    try:
        ...
        if not current:
            return (
                f"Air quality data is currently unavailable "
                f"for '{city}'."
            )
        ...
        current_time = current.get("time", "Unknown time")

        lines = [
            f"Air Quality in {geo['display_name']} "
        ...
        if o3 is not None:
            lines.append(f"  Ozone (O₃)       : {o3:.1f} µg/m³")

        if no2 is not None:
            lines.append(f"  Nitrogen Dioxide : {no2:.1f} µg/m³")

        if co is not None:
            lines.append(f"  Carbon Monoxide  : {co:.1f} µg/m³")

        # Simple AQI interpretation using PM2.5
        if pm25 is not None:
        ...
        return (
            f"Request timed out while fetching air quality "
            f"for '{city}'. Please try again."
        )

    except httpx.HTTPStatusError as e:
        return (
            f"Air quality API error for '{city}': "
            f"HTTP {e.response.status_code}"
        )

    except Exception as e:
        return (
            f"Failed to get air quality for '{city}': "
            f"{type(e).__name__}: {repr(e)}\n"
            f"Trace: {traceback.format_exc(limit=1).strip()}"
        )
